...
export default function LoginPage() {
  ...
  const [applyForm,    setApplyForm]    = useState<ApplyForm>(EMPTY_APPLY);
  ...
  function setField<K extends keyof ApplyForm>(key: K, val: ApplyForm[K]) {
    setApplyForm(prev => ({ ...prev, [key]: val }));
  }

  async function handleApplySubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!applyForm.name.trim() || !applyForm.phone.trim() ||
        !applyForm.depositorName.trim() || !applyForm.googleEmail.trim()) return;
    setApplyLoading(true);
    try {
      await submitApplication({
        name:          applyForm.name.trim(),
        phone:         applyForm.phone.trim(),
        depositorName: applyForm.depositorName.trim(),
        googleEmail:   applyForm.googleEmail.trim(),
        receiptType:   applyForm.receiptType,
        receiptInfo:   applyForm.receiptInfo.trim(),
        plan:          applyForm.plan,
      });
      ...
      <section id="apply" className="bg-[#0e2044] py-20 px-5">
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-[#0d1426] border border-blue-900/50 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleApplySubmit} className="px-6 py-8 space-y-5">
              ...
              <div>
                ...
                <input type="tel" required value={applyForm.phone}
              ...
              <div>
                ...
                <input type="text" required value={applyForm.depositorName}
              ...
              <div>
                ...
                <input type="email" required value={applyForm.googleEmail}
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  {(['없음', '현금영수증', '세금계산서'] as ReceiptType[]).map(type => (
                    <button key={type} type="button" onClick={() => setField('receiptType', type)}
                      className={`flex-1 py-3 text-sm font-bold rounded-xl border transition-all ${
                        applyForm.receiptType === type
                ...
                {applyForm.receiptType !== '없음' && (
                  <input type="text" value={applyForm.receiptInfo}
                    onChange={e => setField('receiptInfo', e.target.value)}
                    placeholder={applyForm.receiptType === '현금영수증' ? '휴대폰 번호 또는 카드 번호' : '사업자 등록번호'}
                    className="w-full bg-blue-950/50 border border-blue-800/60 rounded-xl px-4 py-3.5 text-base text-white placeholder-slate-500 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400/40 transition-colors" />
                )}
              </div>
              ...
              <a href="https://sojangnote.com" target="_blank" rel="noopener noreferrer"
                className="w-full py-5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-black text-xl rounded-2xl transition-all shadow-xl shadow-blue-900/50 flex items-center justify-center gap-2">
                신청하기<svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
      ...
      <footer className="bg-[#080e1f] border-t border-blue-900/40 py-10 px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-xs text-slate-500">
            ...
            <span>|</span>
            <a href="#" className="hover:text-slate-300 underline underline-offset-2 transition-colors">개인정보처리방침</a>
            <span>|</span>
            <a href="http://pf.kakao.com/_LDfqX/chat" target="_blank" rel="noopener noreferrer"
              className="hover:text-slate-300 transition-colors">카카오 문의</a>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-6">
          <div className="bg-white rounded-3xl max-w-sm w-full overflow-hidden shadow-2xl">
            ...
            <div className="px-8 py-10 text-center">
              ...
              <h3 className="text-slate-900 font-extrabold text-2xl mb-2">신청 완료!</h3>
