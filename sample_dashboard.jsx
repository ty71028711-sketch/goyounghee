...
import { MapPin, Search, ExternalLink, Smartphone, CheckCircle2, Navigation, MousePointer2 } from 'lucide-react';

const App = () => {
  const [activeId, setActiveId] = useState(null);

  // 체험용 샘플 데이터 (소장님의 실제 데이터를 넣기 전 맛보기용)
    ...
    <div className="flex flex-col h-screen bg-[#0a0f1e] text-slate-200 font-sans">
      ...
      <header className="bg-slate-900 border-b border-slate-800 p-5 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black italic tracking-tighter">IMJANG MATE <span className="text-blue-400">DEMO</span></h1>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {sampleProperties.map(p => (
              ...
              >
                ...
                {/* 목록 안의 네이버 버튼 (활성화 시에만 노출) */}
                <div className={`mt-3 overflow-hidden transition-all duration-300 ${activeId === p.id ? 'max-h-12 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <button
        ...
        {/* 오른쪽: 지도 (Map Simulation) */}
        <div className="flex-1 relative bg-slate-950 overflow-hidden">
          {/* 가짜 지도 격자 */}
          <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(#475569 1px, transparent 1px)', backgroundSize: '30px_30px' }}></div>
            ...
            >
              ...
              >
                ...
                <div className={`w-2.5 h-2.5 rotate-45 -mt-1.5 border-r-2 border-b-2 ${
                  activeId === p.id ? 'bg-blue-600 border-white' : 'bg-slate-900 border-slate-700 group-hover:border-blue-500'
                }`}></div>
              </div>
            </div>
          ))}
          ...
          {/* 지도 플로팅 정보창 (클릭 시 하단에 등장) */}
          <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 transition-all duration-500 ${activeId ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
            <div className="bg-slate-900 border-2 border-blue-500/50 p-4 rounded-2xl shadow-2xl flex items-center gap-6 min-w-[400px]">
              ...
              <div className="flex-1">
                ...
                <h3 className="text-white font-bold">{sampleProperties.find(p => p.id === activeId)?.name}</h3>
                <p className="text-xs text-slate-500 italic">지도의 핀을 통해 정확한 동선을 파악하세요.</p>
              </div>
              <button
                onClick={() => window.open(sampleProperties.find(p => p.id === activeId)?.naverUrl, '_blank')}
